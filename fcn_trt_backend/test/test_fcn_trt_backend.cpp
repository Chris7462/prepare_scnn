...
public:
  const int input_width_ = 1226;
  const int input_height_ = 370;
  const int num_classes_ = 21;

private:
  const std::string engine_path_ = "fcn_resnet101_370x1226.engine";
...
TEST_F(FCNTrtBackendTest, TestBasicInference)
{
  ...
  EXPECT_EQ(image.cols, 1226);
  EXPECT_EQ(image.rows, 370);
